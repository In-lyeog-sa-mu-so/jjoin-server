...
public class DateUtil {
    ...
    public static String timestampToString(Timestamp date) {
        return String.format("%d%02d%02d", date.getYear(), date.getMonth()+1, date.getDate());
    }
    ...
    public static Timestamp dateWeekStartDate(String date, int weekNum) {
        ...
        int firstDay = targetDate.getDay();
        Calendar cal = Calendar.getInstance();
        cal.setTime(targetDate);

        // 만약 dateWeek가 가리키는 월의 1일이 목요일 이전에 시작한다면 해당 월 1주차에 이전월 날짜 포함
        if (targetDate.getDay() <= 3) {
            cal.add(Calendar.DATE, -firstDay - 1 + 7*(weekNum - 1));
            return new Timestamp(cal.getTime().getTime());
        } else {
            cal.add(Calendar.DATE, 6 - firstDay + 7*(weekNum - 1));
            return new Timestamp(cal.getTime().getTime());
        }
    }
    ...
    public static List<Timestamp> weekDays(String dateWeek) {
        Timestamp startDate = dateWeekStartDate(dateWeek.substring(0, 6), dateWeek.charAt(0) - '0');
        Calendar cal = Calendar.getInstance();
